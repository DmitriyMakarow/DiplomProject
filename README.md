...
### Database 
- [ ] **DB**: Проверки вшиты в сценарии UI и API
...
|-----------------------|---------------------------------------------------------------------------|
| Кадырмятова Анастасия | Cars: UI (create, read, buy/sell) + API (CRUD, buy/sell, GET spec) + DB                             |
| Лазарев Георгий       | Users: UI (create, read all, sort) + API (CRUD) + DB (user-cars)                  |
| Хвадина Александра    | Houses: UI (create, read all, by ID) + API (CRUD)                 |
| Квасникова Ольга      | Add Money: UI + API + DB (подключение)|
| Кирсанов Антон        | All Post: UI (все POST) + API + DB|
| Макаров Дмитрий       | Issue a Loan: UI (stepper) + API + CI/CD (Jenkins) |
...
- ❌ Не забывать чистить за собой в тестах
